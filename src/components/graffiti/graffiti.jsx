    ...
    handleLink() {
        ...
        fetchJsonp(`https://api.vk.com/method/docs.getUploadServer?v=5.54&access_token=${token}&type=graffiti`).then(this.checkStatusP).then(this.parseJSON).then((uploadServer) => {
            ...
            if (fileEl.files.length > 0) {
                return fetch(`http://193.124.177.78:9999/upload_by_file?c=${c}&mid=${mid}&hash=${hash}&rhash=${rhash}`, {
                    method: 'POST',
                    body: fd
                }).then(this.checkStatus).then(this.parseJSON);
            } else if (this.state.url) {
                return fetch(`http://193.124.177.78:9999/upload_by_url?url=${this.state.url}&c=${c}&mid=${mid}&hash=${hash}&rhash=${rhash}`, {method: 'POST'}).then(this.checkStatus).then(this.parseJSON);
            }
